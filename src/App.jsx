...
function ChatTab({ msgs, setMsgs, newMsg, setNewMsg, sendMsg, chatRef, voiceOn, setVoiceOn, activeChatContact, setActiveChatContact, setTab, prevTab, currentUser, socketRef, globalMsgs, setGlobalMsgs, globalMsgStatus, socketReady, readContacts, setReadContacts }) {
  ...
  const myName = currentUser?.name || currentUser?.email || "Moi";
  const fmtSecs = (s) => `${String(Math.floor(s/60)).padStart(2,"0")}:${String(s%60).padStart(2,"0")}`;

  // ── Écouter typing via socket global ──────────────────────
  useEffect(() => {
  ...
  const onTyping = () => {
    if (!socketRef?.current?.connected || !activeC) return;
  ...
  useEffect(() => {
    if (activeChatContact?.name) {
      setActiveC({ ...activeChatContact, initials: activeChatContact.name.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase() });
    }
  }, [activeChatContact?.name]);

  useEffect(() => { chatBottomRef.current?.scrollIntoView({ behavior:"smooth" }); }, [globalMsgs, activeC]);

  const getMsgs = (name) => [
    ...(globalMsgs?.[name] || []),
    ...(globalMsgs?.[myName]?.filter(m => m.to === name) || []),
  ...
  if (!activeC) return (
    <>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:socketReady?"#00E676":"#607080" }}>
          <div style={{ width:7, height:7, borderRadius:"50%", background:socketReady?"#00E676":"#607080" }} />
          {socketReady?"En direct":"Hors ligne"}
        </div>
      </div>

      {/* Conversation initiée depuis profil/offre */}
      {activeChatContact && !allContacts.includes(activeChatContact.name) && (
        <div style={{ background:"#1E88E522", borderRadius:14, padding:14, marginBottom:10, display:"flex", alignItems:"center", gap:12, border:"1px solid #1E88E544", cursor:"pointer" }}
          onClick={()=>setActiveC({ ...activeChatContact, initials: activeChatContact.name.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase() })}>
      ...
      {allContacts.length === 0 && !activeChatContact && (
        <div style={{ textAlign:"center", color:C.sub, padding:"40px 0" }}>
          <div style={{ fontSize:40, marginBottom:12 }}>💬</div>
          <div style={{ fontSize:14, fontWeight:700 }}>Aucune conversation</div>
          <div style={{ fontSize:12, marginTop:6 }}>Contactez un prestataire depuis l'onglet Techs</div>
        </div>
      )}
    </>
  );
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 140px)" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", WebkitOverflowScrolling:"touch", padding:"4px 0", marginBottom:8 }}>
        ...
        {cMsgs.map((m,i) => {
          const status    = m.isMe ? (globalMsgStatus?.[m.id]||m.status||"sent") : null;
